...
Пустое пространство, образованное из пробелов (```U+0020```), горизонтальных табуляций (```U+0009```), возврата каретки (```U+000D```) и перевода строки (```U+000A```), игнорируется,
за исключением случаев, когда им разделяются токены, которые в противном случае были бы объединены в один токен.
