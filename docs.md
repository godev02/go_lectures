...
Следующие правила используются для обозначения определённых категорий символов Юникода:
```
...
Общий комментарий, не содержащий новых строк, равнозначен в коде одному пробелу. Любой другой комментарий действует как переход на новую строку.
...
Пустое пространство, образованное из пробелов (```U+0020```), горизонтальных табуляций (```U+0009```), возврата каретки (```U+000D```) и переводов строки (```U+000A```), игнорируется,
за исключением случаев, когда им разделяются токены, которые в противном случае были бы объединены в один токен. Однако, новая строка или конец файла могут вызвать вставку точки с запятой.
